...
describe('SurrealDBClient', () => {
  ...
  describe('connect', () => {
    ...
    it('should throw error when connection fails', async () => {
      ...
      await expect(client.connect(connectionOptions)).rejects.toThrow(
        'Failed to connect to SurrealDB: Connection failed'
      );
    });
    ...
    it('should throw error when signin fails', async () => {
      ...
      await expect(client.connect(connectionOptions)).rejects.toThrow(
        'Failed to connect to SurrealDB: Signin failed'
      );
    });
    ...
    it('should throw error when use namespace/database fails', async () => {
      ...
      await expect(client.connect(connectionOptions)).rejects.toThrow(
        'Failed to connect to SurrealDB: Use failed'
      );
    });
  ...
  describe('query', () => {
    ...
    it('should execute query with parameters', async () => {
      ...
      expect(mockSurrealInstance.query).toHaveBeenCalledWith(
    ...
    it('should handle query without connection (depends on SurrealDB behavior)', async () => {
      ...
      await expect(disconnectedClient.query('SELECT * FROM user')).rejects.toThrow(
        'Query execution failed: Not connected'
      );
    });
    ...
    it('should handle complex query results', async () => {
      const mockResult = [[{ id: 'user:1', name: 'John' }], [{ id: 'user:2', name: 'Jane' }]];
      mockSurrealInstance.query.mockResolvedValue(mockResult);

      const result = await client.query(
        'SELECT * FROM user; SELECT * FROM user WHERE id = "user:2"'
      );
  ...
  describe('create', () => {
    ...
    it('should handle create without connection', async () => {
      ...
      await expect(disconnectedClient.create('user', { name: 'John' })).rejects.toThrow(
        'Failed to create record: Not connected'
      );
    });
    ...
    it('should handle create operation errors', async () => {
      ...
      await expect(client.create('user', { name: 'John' })).rejects.toThrow(
        'Failed to create record: Create failed'
      );
    });
