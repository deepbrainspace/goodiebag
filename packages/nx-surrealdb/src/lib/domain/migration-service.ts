...
export class MigrationService {
  ...
  async initialize(options: MigrationServiceOptions): Promise<void> {
    ...
    const resolvedOptions: ResolvedMigrationOptions = {
      ...
      namespace: options.namespace
        ? replaceEnvVars(options.namespace)
        : process.env.SURREALDB_NAMESPACE || 'default',
      database: options.database
        ? replaceEnvVars(options.database)
        : process.env.SURREALDB_DATABASE || 'default',
      useTransactions: options.useTransactions ?? true,
      initPath: options.initPath
        ? replaceEnvVars(options.initPath)
        : process.env.MIGRATIONS_PATH || 'database',
    ...
    if (!resolvedOptions.url || !resolvedOptions.user || !resolvedOptions.pass) {
      throw new Error(
        'Missing required configuration. Provide either through options or environment variables.'
      );
    }
  ...
  async findPendingMigrations(
    ...
    for (const moduleId of executionOrder) {
      ...
      for (const file of moduleFiles) {
        ...
        const { canApply } = await repository.canApplyMigration(file.number, file.name, direction);
  ...
  async executeMigrations(
    ...
    if (operation === 'rollback') {
      ...
      if (targetModules && targetModules.length > 0) {
        ...
        this.debug.log(
          `Specific modules requested for rollback (in order): ${modulesToRollback.join(', ')}`
        );
      } else {
      ...
      for (const migration of appliedMigrations) {
        ...
        try {
          ...
          this.debug.log(
            `Warning: Down file not found for ${migration.number}_${migration.name} in ${migration.module}: ${downFilePath}`
          );
      ...
      if (!options.force && operation === 'rollback') {
        ...
        migrationsToProcess = migrationsToProcess.filter(migration => {
          ...
          if (isLocked) {
            this.debug.log(
              `Skipping locked module migration: ${migration.moduleId}/${migration.filename}`
            );
          }
          return !isLocked;
        });
    ...
    for (const migrationFile of migrationsToProcess) {
      ...
      results.push(result);
      this.debug.log(
        `Migration ${migrationFile.filename} result: success=${result.success}, skipped=${result.skipped}`
      );
    ...
    this.debug.log(
      `Final result: success=${finalSuccess}, filesProcessed=${filesProcessed}, filesSkipped=${filesSkipped}, results.length=${results.length}`
    );
  ...
  async validateRollback(targetModules?: string[]): Promise<{
    ...
    for (const moduleId of resolvedModules) {
      ...
      if (!dependencyValidation.canRollback) {
        ...
        if (actualBlockers.length > 0) {
          ...
          warnings.push(
            `Cannot rollback ${moduleId} because it has active dependents with applied migrations: ${actualBlockers.join(
              ', '
            )}`
          );
        }
      ...
      if (hasAppliedMigrations && hasRollbackFiles) {
        const rollbackFileMap = new Map(rollbackFiles.map(rf => [`${rf.number}_${rf.name}`, rf]));
    ...
    if (!options.force && canRollback) {
      const modulesWithAppliedMigrations = migrationChecks.filter(
        check => check.hasAppliedMigrations
      );
  ...
  async getMigrationStatus(targetModules?: string[]): Promise<{
    ...
    const modulesToCheck = targetModules
      ? this.resolveTargetModules(targetModules)
      : resolver.getAllModules();
  ...
  async getFileStatus(resolvedFilenames: ResolvedFilename[]): Promise<
    Array<{
      filename: string;
      moduleId: string;
      number: string;
  ...
  public resolveTargetModules(targetModules: string[]): string[] {
    ...
    if (result.notFound.length > 0) {
      const allModules = this.context?.resolver.getAllModules() || [];
      throw new Error(
        `Module(s) not found: ${result.notFound.join(', ')}. Available modules: ${allModules.join(
          ', '
        )}`
      );
    }
  ...
  public async resolveTargetFilenames(
    ...
    const resolvedTargetModules = targetModules
      ? this.resolveTargetModules(targetModules)
      : undefined;

    const result = await this.patternResolver.resolveFilenames(
      filenamePatterns,
  ...
  public async resolveRollbackFilenames(
    ...
    const result = await this.patternResolver.resolveRollbackFilenames(
      filenamePatterns,
      resolvedTargetModules
    );
  ...
  async findModuleMigrations(
    ...
    try {
      ...
      const migrationFiles = MigrationFileProcessor.filterMigrationFiles(
        files,
        undefined,
        direction
      );
  ...
  private async executeSingleMigration(migrationFile: MigrationFile): Promise<MigrationFileResult> {
    ...
    try {
      ...
      if (options.dryRun) {
        ...
        this.debug.log(
          `   Content preview: ${processedContent.substring(0, 200)}${
      ...
      // Record migration
      await repository.addMigration(
        this.buildMigrationRecord(migrationFile, 'success', executionTimeMs)
      );
      ...
      await repository.addMigration(
        this.buildMigrationRecord(migrationFile, 'fail', executionTimeMs)
      );
  ...
  private async getAppliedMigrations(moduleId: string): Promise<
    Array<{
      number: string;
      name: string;
      direction: 'up' | 'down';
  ...
  > {
    ...
    try {
      ...
      return lastMigrations.map(migration => ({
        number: migration.number || '',
        name: migration.name || '',
