...
export class MigrationRepository {
  ...
  async addMigration(migration: MigrationRecord): Promise<void> {
    ...
    if (this.dryRun) {
      ...
      this.debug.log(
        `   Migration: ${migration.number}_${migration.name}_${migration.direction}.surql`
      );
  ...
  async getMigrationsByDirectionAndPath(direction: string, path: string): Promise<Migration[]> {
    ...
    try {
      ...
      this.debug.log(
        `result[0] length:`,
        Array.isArray(result[0]) ? result[0].length : 'not array'
      );
      this.debug.log(
        `result[0]?.result length:`,
        Array.isArray(result[0]?.result) ? result[0].result.length : 'not array'
      );

      const migrations = ((result[0] as unknown as MigrationRecord[]) || []).map(
        (m: MigrationRecord) => ({
          id: m.id,
          number: m.number,
          name: m.name,
      ...
      this.debug.error(
        `Failed to fetch migrations for direction='${direction}' path='${path}':`,
        error
      );
    ...
    try {
      ...
      const eligibleMigrations = migrations.filter(
        (migration: MigrationRecord) =>
          migration.direction?.toLowerCase() === 'up' &&
          migration.status?.toLowerCase() === 'success'
      );

      this.debug.log(
        `Found ${eligibleMigrations.length} migrations eligible for rollback (latest record is UP+SUCCESS)`
      );
      ...
      this.debug.log(
        `Returning ${result_migrations.length} eligible migrations for rollback (preserving module order, number DESC)`
      );
      return result_migrations;
    } catch (error) {
      this.debug.error(
        `Failed to fetch last migrations for modules [${moduleIds.join(', ')}]:`,
        error
      );
  ...
  async getAllModuleStatusCounts(moduleIds?: string[]): Promise<
    Map<
      string,
      {
        appliedCount: number;
  ...
  > {
    try {
      ...
      for (const [moduleId, moduleMigrations] of migrationsByModule.entries()) {
        ...
        const appliedMigrations = moduleMigrations.filter(
          migration =>
            migration.direction?.toLowerCase() === 'up' &&
            migration.status?.toLowerCase() === 'success'
        );
        ...
        if (appliedMigrations.length > 0) {
          const sortedByDate = appliedMigrations.sort(
        ...
        this.debug.log(
          `Module ${moduleId}: ${appliedMigrations.length}/${moduleMigrations.length} applied`
        );
      }
