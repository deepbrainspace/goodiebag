...
export default async function runExecutor(
  ...
  try {
    ...
    const resolvedTargetModules = rawTargetModules
      ? service.resolveTargetModules(rawTargetModules)
      : undefined;
    debug.log(
      `Resolved target modules: ${resolvedTargetModules ? resolvedTargetModules.join(', ') : 'all'}`
    );

    // Handle filename-specific rollback validation
    if (targetFilenames && targetFilenames.length > 0) {
      try {
        const rollbackResult = await service.resolveRollbackFilenames(
          targetFilenames,
    ...
    if (result.success) {
      if (result.filesProcessed === 0 && result.results.length === 0) {
        logger.info(
          `⚠️  No migrations to rollback - all requested migrations are already rolled back or not yet applied`
        );
