...
impl CredentialsManager {
    ...
    pub async fn get_session_info(&self) -> Result<SessionInfo> {
        ...
        if session_info.is_expired {
            ...
            debug!(
                "Claude session expires in {}",
                Self::format_time_remaining(time_remaining)
            );
        }
...
mod tests {
    ...
    async fn test_format_time_remaining() {
        ...
        assert_eq!(
            CredentialsManager::format_time_remaining(3665000),
            "1h 1m 5s"
        );
        assert_eq!(
            CredentialsManager::format_time_remaining(90065000),
            "1d 1h 1m"
        );
    }
