use crate::{config::manager::ConfigurationManager, error::*, sync::SyncService, utils::systemd::SystemdManager};
    ...
    if state.targets.is_empty() {
        ...
        for target in &state.targets {
            ...
            if target.last_sync_time > 0 {
                let sync_time = chrono::DateTime::from_timestamp(target.last_sync_time, 0)
                    .unwrap_or_default()
                    .format("%Y-%m-%d %H:%M:%S UTC");
