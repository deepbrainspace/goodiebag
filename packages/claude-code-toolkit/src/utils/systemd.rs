...
WantedBy=default.target"#,
  ...
  pub async fn status(&self) -> Result<String> {
    let output = Command::new("systemctl").args(["--user", "status", SERVICE_NAME]).output().await?;
  ...
  pub async fn logs(&self, lines: usize) -> Result<String> {
    let output = Command::new("journalctl")
      .args(["--user", "-u", SERVICE_NAME, "-n", &lines.to_string(), "--no-pager"])
      .output().await?;
    ...
    if output.status.success() {
      ...
      Err(ClaudeCodeError::Systemd(format!("Failed to get logs: {}", error_msg)))
    }
  }
  ...
  async fn run_systemctl(&self, args: &[&str]) -> Result<()> {
    ...
    if output.status.success() {
      ...
      Err(ClaudeCodeError::Systemd(format!("systemctl command failed: {}", error_msg)))
    }
  }
}
