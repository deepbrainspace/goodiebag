...
use crate::traits::{
    Credentials, SecretManager, SecretMapping, SecretProvider, SyncResult, Target,
};
...
impl ProviderRegistry {
    ...
    pub async fn initialize_provider(
        ...
        if !provider.is_configured().await? {
            ...
            return Err(crate::error::ClaudeCodeError::Generic(format!(
...
impl SecretManager for ProviderRegistry {
    ...
    async fn sync_credentials_to_targets(
        ...
        for (provider_name, provider) in &self.providers {
            match provider.sync_secrets(&secrets, targets).await {
                ...
                Err(e) => {
                    ...
                    total_result
                        .errors
                        .push(format!("Provider {}: {}", provider_name, e));
                }
            }
        }
        ...
        info!(
            ...
            total_result.succeeded, total_result.failed
        );
    ...
    async fn validate_targets(&self) -> Result<HashMap<String, bool>> {
        ...
        for (provider_name, provider) in &self.providers {
            ...
            match provider.validate_access(&targets).await {
                ...
                Err(e) => {
                    warn!(
                        "Failed to validate targets for provider {}: {}",
                        provider_name, e
                    );
                }
            }
