...
pub async fn handle_install() -> Result<()> {
    ...
    println!(
        "{}",
        style("1. Add organizations: claude-code org add <org-name>").dim()
    );
    println!(
        "{}",
        style("2. Add repositories: claude-code repo add <owner/repo>").dim()
    );
...
pub async fn handle_disable() -> Result<()> {
    println!(
        "{}Disabling Claude Code sync daemon auto-start...",
        style("❌").red()
    );
